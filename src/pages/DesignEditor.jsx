...
const DESIGN_TYPES = ['bracelet', 'necklace', 'ring', 'earrings', 'other']
...
export default function DesignEditor() {
  ...
  const [type, setType] = useState('bracelet')
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl p-4 space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Gold wire bracelet"
            />
          </div>
        ...
        <div className="bg-white rounded-2xl p-4">
          <div className="flex justify-between items-center mb-3">
            <h2 className="font-semibold text-gray-800 text-sm">Materials</h2>
            {availableMaterials.length > 0 && (
              <button onClick={() => setShowAddMaterial(!showAddMaterial)}
                className="text-xs text-rose-500 font-medium">
                {showAddMaterial ? 'Cancel' : '+ Add'}
              </button>
            )}
          </div>
              ...
              {selectedMaterialId && (
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder={`Qty in ${allMaterials.find(m => m.id === selectedMaterialId)?.unit ?? 'units'}`}
          ...
          {designMaterials.length === 0 ? (
            <div className="text-center text-gray-400 py-4 text-sm">
              {allMaterials.length === 0
                ? 'Add materials to your stock first.'
        ...
        {/* Costs & Pricing */}
        <div className="bg-white rounded-2xl p-4 space-y-4">
          <h2 className="font-semibold text-gray-800 text-sm">Costs & Pricing</h2>
          ...
          <div>
            <label className="text-xs font-medium text-gray-600 block mb-1">Sale price (€)</label>
            <input
              type="number"
              step="0.01"
              min="0"
