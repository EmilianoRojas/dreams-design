...
const TYPES = ['bracelet', 'necklace', 'ring', 'earrings', 'other']
...
export default function Designs() {
  ...
  const filtered = filterType === 'all' ? designs : designs.filter(d => d.type === filterType)

  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-xl font-bold text-gray-800">Designs</h1>
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-400 py-16">
          ...
          <p className="text-sm">
            {filterType !== 'all' ? 'No designs of this type yet.' : 'No designs yet. Create your first one!'}
          </p>
        </div>
      ) : (
              ...
              <div key={d.id}
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    {d.sale_price != null ? (
                      ...
                      <div className="text-sm text-gray-300">No price set</div>
                    )}
                  </div>
                </div>
                ...
                {(d.design_materials?.length ?? 0) > 0 && (
                  <div className="mt-2 text-xs text-gray-400">
                    {d.design_materials.length} material{d.design_materials.length !== 1 ? 's' : ''}
                  </div>
                )}
              </div>
            )
